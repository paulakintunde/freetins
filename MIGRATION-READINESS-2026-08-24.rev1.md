...
> **Status as of 24 August 2026, end of day.** Everything on this checklist is closed
> in the codebase except the verification re-sourcing, which is editorial work and
> cannot be closed by a deploy. One item — attaching the custom domain — is a
> Cloudflare dashboard action and is listed under 6.3 rather than here.

**Do today, regardless of the cutover date:**

- [x] Add `X-Robots-Tag: noindex` to the `freetins.pages.dev` hostname (4.3) — `public/_headers`, scoped to `:project.pages.dev` and its preview subdomains so the custom domain never receives it

**Blockers — cutover cannot proceed until these are closed:**

- [x] Decide the canonical host. **`www.freetins.com`**, per the recommendation in 4.2
- [x] Change `site:` in `astro.config.mjs` **and all 12 hardcoded literals** (4.2) — the literals are gone, not merely edited: JSON-LD now derives its `@id`s and URLs from `siteOrigin` in `src/data/site.ts`, so the host cannot drift from the canonical tag again
- [x] Add the 3 legal-page redirects: `/privacy-policy/`, `/terms-of-use/`, `/dcma/` (4.1)
- [x] Decide the 5 "publish later" URLs: 301 to the nearest live hub, replaceable with a direct 301 once each page ships (4.1)
- [x] Map the 4 WP category archives to hubs
- [x] Return `410 Gone` for the 15 deliberate removals — server-rendered routes under `src/pages/`, listed in `src/data/gone.ts`, since `_redirects` cannot express 410
- [x] Add the HSTS header (4.4) — `max-age=31536000; includeSubDomains`, no `preload`
- [ ] **Confirm official channels and re-source codes for the 12 published games (5)** — the one open blocker. Editorial work; no code change closes it

**Should ship in the same release:**

- [x] Move the editorial filter script out of the inline block (4.5) — now `src/scripts/element-index.ts`, bundled
- [x] Fix `Header.astro` `isActive` trailing slashes (4.6)
- [x] Add a desktop header search entry point (4.6)
- [x] Unify the contact address across all three surfaces (4.6) — `support@freetins.com` everywhere, from `contactEmail`

**Verified against a `wrangler pages dev` run of the production build:** all 15 removals
return 410, all 30 redirect rules resolve to a live target, no existing page changed
status, `Strict-Transport-Security` is present on every response, and `X-Robots-Tag:
noindex` appears only when the request carries a `pages.dev` `Host`.
