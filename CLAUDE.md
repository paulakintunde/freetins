...
pnpm check:vocabulary          report retired vocabulary (--strict fails; strict from Step 1a)
pnpm check:routes              crawl dist/ for links, sitemap and noindex agreement (after build)
```

CI runs typecheck, lint, check:data, test, check:content --strict, check:vocabulary,
build and check:routes, in that order. `docs/TESTING.md` says what each proves.
...
- `docs/WRITER-CONTRACT.md` — the dataset-page contract as it stands (v1), until
  `docs/WRITER-CONTRACT-v2.md` supersedes it.
